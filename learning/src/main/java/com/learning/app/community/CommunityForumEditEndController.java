...
public class CommunityForumEditEndController implements Execute {
	...
	public Result execute(HttpServletRequest request, HttpServletResponse response)
		...
		// 게시글 수정 처리
		communityDAO.EditEnd(map);
		communityDTO.setForumNumber(Integer.parseInt(multipartRequest.getParameter("postNum")));
		...
		while (fileNames.hasMoreElements()) {
			...
			if (fileOriginalName != null) {
				// 기존 파일 정보 가져오기
				List<FileDTO> fileList = fileDAO.select(communityDTO.getForumNumber());
				...
				if (!fileList.isEmpty()) {
					...
					// DB에서 기존 파일 정보 삭제
					fileDAO.delete(communityDTO.getForumNumber()); // 기존 파일 정보 삭제
