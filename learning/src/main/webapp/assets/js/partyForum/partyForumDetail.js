...
/*    console.log("확인 postNum:", postNum);
    ...
    if (submitBtn) {
        submitBtn.addEventListener("click", async function() {
            ...
            try {
                const response = await fetch(`${contextPath}/app/partyForum/partyForumCommentAdd.fo`, {
                    method: "POST",
        ...
        commentList.addEventListener("click", async function(event) {
            if (event.target.classList.contains("comment-delete")) {
                ...
                if (confirm("댓글을 삭제하시겠습니까?")) {
                    try {
                        const response = await fetch(`${contextPath}/app/partyForum/partyForumCommentDelete.fo?replyNumber=${replyNumber}`, { method: "GET" });
                        const result = await response.json();
                        if (result.status === "success") {
    ...
    async function loadReplies() {
        try {
            const response = await fetch(`${contextPath}/app/partyForum/partyForumCommentList.fo?postNum=${postNum}`);
    ...
    if (applyBtn) {
        applyBtn.addEventListener("click", async function() {
            if (confirm("참가 신청 하시겠습니까?")) {
                try {
                    const response = await fetch(`${contextPath}/app/partyForum/partyForumApply.fo?postNum=${postNum}`, { method: "GET" });
                    const result = await response.json();
